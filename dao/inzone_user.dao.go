...
func (a *InzoneUserRepo) GetByCookie(ctx context.Context, cookie string) (*pb.InzoneUser, error) {
	one := &model.InzoneUser{}
	err := getSession(ctx).Model(&model.InzoneUser{}).Where("cookie=?", cookie).First(one).Error
	if err == gorm.ErrRecordNotFound {
		return nil, nil
	}
	if err != nil {
